...
Private Const SH_BASE As String = "Dashboard CP"

' Modo de agregacion de la linea mensual:
'   gPond = False -> promedio simple   (hoja "Dashboard CP")
'   gPond = True  -> promedio ponderado por monto colocado (hoja "Dashboard CP Pond")
Private gSH As String
Private gPond As Boolean
Private gSuf As String
...
Sub CrearDashboardSpreadsCP()
    ...
    gSuf = ""
    gSH = SH_BASE
    ConstruirDashboardCP
End Sub

Sub CrearDashboardPonderado()
    ' promedio ponderado por monto colocado
    gPond = True
    gSuf = "P"
    gSH = SH_BASE & " Pond"
    ConstruirDashboardCP
...
Sub CrearAmbosDashboards()
    CrearDashboardSpreadsCP
    CrearDashboardPonderado
End Sub

Private Sub ConstruirDashboardCP()
    ...
    ws.Range("A2").Value = "  " & IIf(gPond, "Promedio mensual PONDERADO POR MONTO", "Promedio mensual simple") & " por rating - PEN - DB Histórica CP"
    ...
    BorrarNombre wb, "lstSectores" & gSuf
    BorrarNombre wb, "lstEmisores" & gSuf
    BorrarNombre wb, "lstMonedas" & gSuf

    wb.Names.Add Name:="lstSectores" & gSuf, _
        RefersTo:="='" & gSH & "'!$" & ColL(110) & "$2:INDEX('" & _
        gSH & "'!$" & ColL(110) & "$2:$" & ColL(110) & "$300,COUNTA('" & _
        gSH & "'!$" & ColL(110) & "$2:$" & ColL(110) & "$300))"

    wb.Names.Add Name:="lstEmisores" & gSuf, _
        RefersTo:="='" & gSH & "'!$" & ColL(112) & "$2:INDEX('" & _
        gSH & "'!$" & ColL(112) & "$2:$" & ColL(112) & "$900,COUNTA('" & _
        gSH & "'!$" & ColL(112) & "$2:$" & ColL(112) & "$900))"

    wb.Names.Add Name:="lstMonedas" & gSuf, _
        RefersTo:="='" & gSH & "'!$" & ColL(120) & "$2:INDEX('" & _
        gSH & "'!$" & ColL(120) & "$2:$" & ColL(120) & "$50,COUNTA('" & _
        gSH & "'!$" & ColL(120) & "$2:$" & ColL(120) & "$50))"

    If cDiv > 0 Then
        PonerLista ws.Range("I4"), "=" & "lstMonedas" & gSuf
    End If

    PonerLista ws.Range("D4"), "=" & "lstSectores" & gSuf
    PonerLista ws.Range("E4"), "=" & "lstEmisores" & gSuf
    PonerLista ws.Range("F4"), "=" & "lstEmisores" & gSuf
    ...
    For m = 0 To 2
        ...
        If m < 2 Then
            ...
            If gPond And TieneNombre(wb, "dbMonto") Then

                cMes = "(dbRating=" & ColL(b) & "$2)*" & _
                       "(dbFecha>=$AH3)*(dbFecha<EDATE($AH3,1))*" & _
                       "IF($D$4=""(todos)"",1,dbSector=$D$4)" & sPF

                ws.Cells(3, b).Formula = _
                    "=IF($AH3=0,0,IF(" & ColL(b) & "$1=0,0," & _
                    "IFERROR(" & fc & "*SUMPRODUCT((" & cMes & ")*dbMonto*" & nm & ")/" & _
                    "SUMPRODUCT((" & cMes & ")*dbMonto),0)))"
            ...
            If gPond And TieneNombre(wb, "dbMonto") Then
                ...
                ' solo cuentan las filas con monto: si no hay monto no se puede ponderar
                cMes = "(dbRating=" & ColL(b) & "$2)*" & _
                       "(dbFecha>=$AH3)*(dbFecha<EDATE($AH3,1))*" & _
                       "IF($D$4=""(todos)"",1,dbSector=$D$4)" & sPF
        ...
        Else
            ...
            If gPond And TieneNombre(wb, "dbMonto") Then
                ...
                cMes = "(dbFecha>=$AH3)*(dbFecha<EDATE($AH3,1))*" & _
                       "IF($D$4=""(todos)"",1,dbSector=$D$4)" & sPF

                ws.Cells(3, b).Formula = _
                    "=IF($AH3=0,0,IFERROR(" & fc & _
                    "*SUMPRODUCT((" & cMes & ")*dbMonto*" & nm & ")/" & _
                    "SUMPRODUCT((" & cMes & ")*dbMonto),0))"
            ...
            If gPond And TieneNombre(wb, "dbMonto") Then
                ...
                ws.Cells(3, b + 5).Formula = _
                    "=IF($AH3=0,0,SUMPRODUCT((" & cMes & ")*(dbMonto>0)))"
    ...
    For m = 0 To 2
        ...
        ch.HasTitle = True
        ch.ChartTitle.Formula = "='" & gSH & "'!$AE$" & (m + 1)
